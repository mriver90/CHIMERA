...
##############################################################################################
def conceptid_for_input(data, engine):
    import pandas as pd
    ...
    for group_name, group_data in data.groupby('sheet_name'):
        ...
        for index, row in group_data.iterrows():
            ...
            query = """SELECT DISTINCT CONCEPT_CODE, CONCEPT_ID, VOCABULARY_ID, DOMAIN_ID, STANDARD_CONCEPT AS FLAG_STANDARD_CONCEPT
                        FROM concept
...
def standard_code_mapping(df1, engine):
    import pandas as pd
    ...
    for group_identifier, group_data in df1.groupby('group_identifier'):
        ...
        # SQL query with columns in lowercase
        sql_base_table = """SELECT DISTINCT C.CONCEPT_ID AS concept_id,
            ...
            ORDER BY C.CONCEPT_ID;""".format(', '.join(['?'] * len(concept_id_list)))
        
        # Execute the SQL query
        result_sql_base_table = pd.read_sql(sql_base_table, engine, params=tuple(concept_id_list))
        
        # Debugging: Check column names in lowercase
        print(result_sql_base_table.columns)
        
        # Add the group identifier to the result
        result_sql_base_table['group_identifier'] = group_identifier
        
        # Ensure 'concept_id' is a string for merging
        result_sql_base_table['concept_id'] = result_sql_base_table['concept_id'].astype(str)
        
        # Merge additional flags from group data
    ...
    result_df = pd.concat(group_result_dataframes)
    ...
    # Rename 'group_identifier' to 'sheet_name' for clarity
    result_df = result_df.rename(columns={'group_identifier': 'sheet_name'})
    
    # Reordering and renaming columns for the final output
    ...
    # Sort the values by 'sheet_name' and 'concept_code'
    result_df = result_df.sort_values(by=['sheet_name', 'concept_code'])
    
    # Reset the index after sorting
    result_df = result_df.groupby('sheet_name', group_keys=False).apply(lambda x: x.reset_index(drop=True))
