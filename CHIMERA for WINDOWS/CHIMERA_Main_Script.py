    ...
    if uploaded_file is not None:
        ...
        if flag_atlas_login == 0:
            ...
            if flag_cleaned_data == 1:
                option = st.selectbox("Select Option:",['', 'ATHENA Vocabularies', 'CDM Database'])

                if option == '':
                    st.warning("Please select an option.")

                if option == 'ATHENA Vocabularies':
                    concept = "concept"
                    concept_relationship = "concept_relationship"
                    connection_string = 'sqlite:///chimera.db'
                    db_type = None

                if option == 'CDM Database':    
                    db_type = st.selectbox("Select Database Type:", ["RedShift", "MySQL", "SQLite", "Oracle", "SQL Server"])
                    ...
                    colxxx, colyyy = st.columns(2)
                    with colxxx:
                        db_user = st.text_input("Username")
                    with colyyy:
                        db_password = st.text_input("Password", type="password")

                    concept = f"{schema_name}.concept"
                    concept_relationship = f"{schema_name}.concept_relationship"
                    ...
                    if db_type == "RedShift":
                       ...
                       connection_string = f"sqlite:///{db_name}"  # For SQLite, db_name is the file path
        
                    elif db_type == "Oracle":
                       connection_string = f"oracle+cx_oracle://{db_user}:{db_password}@{db_host}:{db_port}/{db_name}"
                ...
                try:
                    ...
                    with engine.connect() as connection:
                        result = pd.read_sql(f"SELECT * FROM {concept} LIMIT 10", con=connection)
                        if not result.empty:
            ...
            st.text("")
            if connection_successful == 1 and st.button("Standard Code Mapping"):
                base_table_df = ws.conceptid_for_input(cleaned_data, engine, concept, db_type)
                st.dataframe(base_table_df)
                mapped_table_df = ws.standard_code_mapping(base_table_df, engine, concept, concept_relationship, db_type)
